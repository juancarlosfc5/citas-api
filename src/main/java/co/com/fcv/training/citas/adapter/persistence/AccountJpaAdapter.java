...
class AccountJpaAdapter implements Ports.Accounts {
    ...
    public Optional<Account> byId(UUID id) { return users.findById(id.toString()).map(this::toDomain); }

    public Account save(Account account) {
        UserEntity entity = new UserEntity();
        entity.id = account.id().toString();
        ...
        entity.roles = account.roles().stream()
                .map(name -> roles.findByName(name).orElseThrow(() -> new IllegalStateException("Rol faltante")))
                .collect(Collectors.toSet());
        return toDomain(users.saveAndFlush(entity));
    }

    private Account toDomain(UserEntity e) {
        return new Account(UUID.fromString(e.id), e.firstName, e.lastName, e.documentType,
                e.documentNumber, e.email, e.phone, e.passwordHash,
                e.roles.stream().map(role -> role.name).collect(Collectors.toSet()));
    }
}
